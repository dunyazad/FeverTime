...
PointCloudAlgorithm_Laplacian::PointCloudAlgorithm_Laplacian()
{
...
__global__ void Kernel_Laplacian(HashMapInfo info, float3* positions, float3* normals, uchar4* colors, size_t numberOfPoints)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx >= numberOfPoints) return;
...
void PointCloudAlgorithm_Laplacian::RunAlgorithm(DevicePointCloud* pointCloud)
{
...

	{
		...
		Kernel_Laplacian << <gridOccupied, blockSize >> > (
...
void PointCloudAlgorithm_Laplacian::RunAlgorithm(HostPointCloud* pointCloud)
{
	//nvtxRangePushA("Clustering");
...
void PointCloudAlgorithm_Laplacian::IncreaseParameter()
{
}

void PointCloudAlgorithm_Laplacian::DecreaseParameter()
{
}
